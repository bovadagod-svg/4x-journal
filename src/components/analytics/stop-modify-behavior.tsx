 ...
 *
 * Classifies each SL Replaced event as BE / Trail / Loose and each TP Replaced
 * event as Wider / Tighter using the same rules the Trade Detail Drawer uses.
 * Aggregated per outcome bucket: a 2× gap on "Loose stops" between losers and
 * winners is the canonical hope-trading signature.
 *
...
/**
 * Classify SL/TP modifications on a single trade's lifecycle. Mirrors the
 * Trade Detail Drawer's `enrichEvents` classification logic — kept inline
 * (~30 lines) rather than abstracted because the drawer's version carries
 * UI-specific enrichments we don't want here. If a third consumer appears,
 * extract a shared helper.
 */
function classifyTrade(events: LifecycleEvent[], ctx: { side: "long" | "short"; entryPrice: number }) {
  ...
  for (const e of chrono) {
    ...
    if (isReplaced) {
      ...
      if (e.stopLoss != null && prevSL == null) prevSL = e.stopLoss
      if (e.takeProfit != null && prevTP == null) prevTP = e.takeProfit
    }
  }
